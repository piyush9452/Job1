...
export default function ApplicationDetailsModal({ application, onClose }) {
  ...
  // Helper to style the status section
    ...
    switch (status?.toLowerCase()) {
      ...
      case "hired":
        return {
          ...
          title: "You're Hired!",
          message:
            "Congratulations! The HR team has accepted your application.",
        };
      case "rejected":
        return {
      ...
      case "interview":
        return {
          ...
          message:
            "You have been shortlisted for an interview. Check your email for details.",
        };
      default: // applied / shortlisted
        return {
      ...
      >
        ...
        <div className="flex justify-between items-start p-6 border-b border-gray-100 bg-white">
          <div className="flex gap-4">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight line-clamp-1">
                {job.title}
              </h2>
        ...
        <div className="overflow-y-auto p-6 md:p-8 space-y-8 custom-scrollbar">
          {/* 1. Status Section */}
          <div
            className={`p-5 rounded-2xl border ${statusUI.color} flex flex-col gap-3`}
          >
            <div className="flex items-center gap-3 font-bold text-lg">
              {statusUI.icon}
              {statusUI.title}
            </div>
            <p className="text-sm font-medium leading-relaxed opacity-90 pl-9">
              {statusUI.message}
            </p>
            <div className="mt-2 pt-3 border-t border-black/5 flex items-center gap-2 text-xs font-semibold opacity-70 pl-9">
              <Calendar size={12} />
